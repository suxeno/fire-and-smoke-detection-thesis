...
def count_parameters(model):
    ...
    Useful for comparing DETR vs DETR-SLIC model sizes.
    
    Args:
        model: PyTorch model
    
    Returns:
        dict: {'total': int, 'trainable': int, 'non_trainable': int}
    """
...
class AverageMeter(object):
    ...
    Simpler alternative to SmoothedValue for basic metric tracking.
    """
    def __init__(self):
        self.reset()
...
def get_device(device_str='cuda'):
    """
    Get torch device, with fallback to CPU if CUDA not available.
    
    Args:
        device_str: Device string ('cuda', 'cuda:0', 'cpu')
...
def get_gpu_memory_usage():
    ...
    Returns:
        dict: {'allocated': float, 'cached': float, 'max_allocated': float}
    """
...
def save_checkpoint(state, filepath, is_best=False, max_keep=5):
    """
    Save checkpoint with optional best model tracking and cleanup of old checkpoints.
    
    Args:
        state: Dictionary containing model state, optimizer state, etc.
        filepath: Path to save checkpoint
        is_best: Whether this is the best model so far
        max_keep: Maximum number of checkpoints to keep (0 = keep all)
    """
    import os
    ...
    # Cleanup old checkpoints if max_keep is set
    if max_keep > 0:
...
def load_checkpoint(filepath, model, optimizer=None, scheduler=None):
    ...
    Returns:
        dict: Checkpoint dictionary with epoch, metrics, etc.
    """
