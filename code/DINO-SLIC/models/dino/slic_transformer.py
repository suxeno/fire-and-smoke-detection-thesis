...
class SLICTransformer(nn.Module):
    ...
    def gen_encoder_output_proposals_from_centroids(
        ...
        for lvl, count in enumerate(level_counts):
            ...
            # Clamp centroids to valid sigmoid range to reduce inf-masked proposals
            level_centroids = level_centroids.clamp(min=0.01, max=0.99)
        ...
        # Inverse sigmoid
        output_proposals = torch.log(output_proposals / (1 - output_proposals + 1e-6))
    ...
    def forward(
        ...
        if self.two_stage_type == 'standard':
            ...
            # Mask out invalid proposals (inf coordinates) from top-k selection
            # so they can never be selected — prevents degenerate boxes downstream
            invalid_proposal_mask = torch.isinf(output_proposals).any(dim=-1)  # [bs, N]
            enc_outputs_class_for_topk = enc_outputs_class.clone()
            enc_outputs_class_for_topk.masked_fill_(invalid_proposal_mask.unsqueeze(-1), float('-inf'))

            if self.debug:
                n_invalid = invalid_proposal_mask.sum(dim=1).float()
                print(f"[SLICTransformer] Proposals: total={output_proposals.shape[1]}, "
                      f"invalid(mean)={n_invalid.mean().item():.1f}, "
                      f"invalid(max)={n_invalid.max().item():.0f}")

            # Clamp topk to available valid tokens
            n_valid = (~padding_mask).sum(dim=1).min().item()
            # Ensure at least 1 query is selected even if no tokens are valid
            ...
            # Clamp sigmoid output to valid range to prevent edge-case degenerate boxes
