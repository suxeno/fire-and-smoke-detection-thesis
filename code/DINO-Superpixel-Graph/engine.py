...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in pbar:
        ...
        with torch.amp.autocast('cuda', enabled=args.amp):
            ...
            # ── NaN guard: detect NaN in model outputs before loss ──
            _nan_in_output = False
            if torch.isnan(outputs['pred_boxes']).any():
                _nan_in_output = True
                _nan_count = torch.isnan(outputs['pred_boxes']).sum().item()
                _total = outputs['pred_boxes'].numel()
                print(f"\n[NaN GUARD iter {_cnt}] NaN in pred_boxes: "
                      f"{_nan_count}/{_total} elements. Skipping batch.")
                # Dump diagnostics for the first occurrence
                if hasattr(model, 'module'):
                    _m = model.module
                else:
                    _m = model
                if hasattr(_m, 'transformer') and hasattr(_m.transformer, '_last_debug'):
                    print(f"  Transformer debug: {_m.transformer._last_debug}")
                for ti, t in enumerate(targets):
                    print(f"  Target[{ti}]: labels={t['labels'].tolist()}, "
                          f"n_boxes={len(t['labels'])}")

            if _nan_in_output:
                # Skip this batch entirely — no backward, no optimizer step
                optimizer.zero_grad()
                _cnt += 1
                continue
        ...
        if args.amp:
            ...
            if max_norm > 0:
                ...
                if not math.isfinite(grad_norm.item()):
                    print(f"\n[NaN GUARD iter {_cnt}] Non-finite grad norm: {grad_norm.item()}. "
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False, args=None, logger=None):
    ...
    for samples, targets in pbar:
        ...
        # Update tqdm progress bar
        pbar.set_postfix({
            'loss': f"{sum(loss_dict_reduced_scaled.values()).item():.4f}"
        })
