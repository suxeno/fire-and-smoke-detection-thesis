...
class DINO(nn.Module):
    ...
    def _reset_parameters(self):
        # init input_proj (skip for SLIC — no Conv2d projections)
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        # DN query preparation (shared between SLIC and standard)
        ...
        if self.is_graph:
            ...
            hs, reference, hs_enc, ref_enc, init_box_proposal = self.transformer(
                backbone_output, input_query_bbox, input_query_label, attn_mask
            )
        else:
            # Standard CNN path: backbone outputs spatial feature maps
        ...
        for dec_lid, (layer_ref_sig, layer_bbox_embed, layer_hs) in enumerate(zip(reference[:-1], self.bbox_embed, hs)):
            ...
            # Clamp reference to valid sigmoid range before inverse_sigmoid
            # to prevent NaN/inf from edge-case degenerate proposals
            layer_ref_sig = layer_ref_sig.clamp(min=1e-4, max=1 - 1e-4)
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        loss_ce = sigmoid_focal_loss(src_logits, target_classes_onehot, num_boxes, alpha=self.focal_alpha, gamma=1) * src_logits.shape[1]
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes, not_to_xyxy=False, test=False):
        ...
        # Clamp num_select to available predictions (SLIC produces variable query counts)
...
def build_dino(args):
    ...
    if args.backbone == 'graph':
        transformer = SLICTransformer(
            ...
            query_dim=4,
            learnable_tgt_init=getattr(args, 'learnable_tgt_init', True),
