...
def build_backbone(args):
    ...
    Useful args:
        ...
        - lr_backbone: 
        - dilation
        - return_interm_indices: available: [0,1,2,3], [1,2,3], [3], [0,1,2] (slic)
    ...
    if args.backbone == 'graph':
        # Graph (GAT) superpixel backbone — returns token sequences, not spatial maps.
        # Position encoding is handled by SLICTransformer, not by Joiner.
        n_segments = getattr(args, 'slic_n_segments', [400, 200, 100])
        compactness = getattr(args, 'slic_compactness', 10.0)
        sigma = getattr(args, 'slic_sigma', 1.0)

        backbone = GraphFeatureExtractor(
            n_segments_per_level=n_segments,
            compactness=compactness,
            sigma=sigma,
            output_dim=args.hidden_dim,
            cnn_out_channels=getattr(args, 'cnn_out_channels', 64),
            gcn_hidden_dim=getattr(args, 'gcn_hidden_dim', 128),
            gcn_num_layers=getattr(args, 'gcn_num_layers', 3),
            gcn_edge_dim=getattr(args, 'gcn_edge_dim', 32),
            gcn_heads=getattr(args, 'gcn_heads', 4),
        )
        # Return backbone directly — no Joiner, no position_embedding
        backbone.num_channels = backbone.num_channels  # list of [256, 256, 256]
        return backbone
