...
def crop(image, target, region):
    ...
    if "slic_maps" in target:
        for k in target['slic_maps']:
            target['slic_maps'][k] = target['slic_maps'][k][i:i + h, j:j + w]
...
def hflip(image, target):
    ...
    if "slic_maps" in target:
        for k in target['slic_maps']:
            target['slic_maps'][k] = target['slic_maps'][k].flip(-1)
...
def pad(image, target, padding):
    ...
    if "slic_maps" in target:
        for k in target['slic_maps']:
            target['slic_maps'][k] = torch.nn.functional.pad(target['slic_maps'][k], (0, padding[0], 0, padding[1]), value=-1)
