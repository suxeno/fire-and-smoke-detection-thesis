...
class DETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, aux_loss=False, slic_n_segments=200, pooling_type='mean'):
    ...
    def pool_superpixel_features(self, features, mask, slic_maps, n_segments, pooling_type='mean'):
        ...
        for b in range(B):
            ...
            if s_map is None:
                ...
                s_map_down = F.interpolate(s_map[None, None].float(), size=(valid_H, valid_W), mode='nearest')
                batched_slic_down[b, :valid_H, :valid_W] = s_map_down[0, 0].long()
        
        feat_flat = features.flatten(2) # [B, C, H*W]
        slic_flat = batched_slic_down.flatten(1) # [B, H*W]
        
        y_grid, x_grid = torch.meshgrid(torch.arange(H, device=device), torch.arange(W, device=device), indexing='ij')
        y_grid = y_grid.flatten() # [H*W]
        x_grid = x_grid.flatten() # [H*W]
        
        for b in range(B):
            unique_sps = torch.unique(slic_flat[b])
            unique_sps = unique_sps[unique_sps != -1]
            unique_sps = unique_sps[unique_sps < n_segments]
            
            for sp_id in unique_sps:
                sp_mask = (slic_flat[b] == sp_id) # [H*W]
                if not sp_mask.any():
                    continue
                ...
                valid_y = y_grid[sp_mask].float()
                valid_x = x_grid[sp_mask].float()
                pooled_pos[b, 0, sp_id] = valid_y.mean() / H
                pooled_pos[b, 1, sp_id] = valid_x.mean() / W
                
        return pooled_features, pooled_mask, pooled_pos
    ...
    def forward(self, samples: NestedTensor, targets=None, debug=False):
        ...
        features, _ = self.backbone(samples) # Skip pos from grid

        src, mask = features[-1].decompose()
        assert mask is not None
        
        slic_maps = [t.get('slic_maps', {}) for t in targets] if targets is not None else []
        pooled_src, pooled_mask, pooled_pos = self.pool_superpixel_features(src, mask, slic_maps, self.slic_n_segments, self.pooling_type)
        
        # [B, C, N] -> [B, C, N, 1] for conv2d
        pooled_src_2d = pooled_src.unsqueeze(-1)
        proj_src = self.input_proj(pooled_src_2d) # [B, 256, N, 1]
        
        # Sine encoding for superpixel centroids
        # pooled_pos: [B, 2, N] -> [B, 2, N, 1]
        pooled_pos_2d = pooled_pos.unsqueeze(-1)
        # self.transformer expects pos_embed of shape [B, 256, N, 1] matching proj_src
        from .position_encoding import PositionEmbeddingSuperpixel
        if not hasattr(self, 'pos_embed_superpixel'):
            self.pos_embed_superpixel = PositionEmbeddingSuperpixel(self.transformer.d_model // 2).to(src.device)
            
        pos_embed = self.pos_embed_superpixel(pooled_pos_2d, debug=debug) # [B, 256, N, 1]
        
        pooled_mask_2d = pooled_mask.unsqueeze(-1) # [B, N, 1]
        
        hs = self.transformer(proj_src, pooled_mask_2d, self.query_embed.weight, pos_embed)[0]
