"""
Report Generation Script for DETR Model
========================================
Generates comprehensive Excel reports of training, validation, and test metrics
for thesis writing and model comparison.

Usage:
    python report_generator.py --output_dir /path/to/outputs --experiment_name "Pretrained-2"
    
Outputs:
    - report_{timestamp}.xlsx with multiple sheets:
        - Summary: Single row with key metrics
...
class ReportGenerator:
    """Generate Excel reports from DETR training outputs."""
    ...
    def __init__(self, output_dir: str, experiment_name: str):
        ...
        Args:
            ...
            experiment_name: Name of the experiment (e.g., "Pretrained-2")
        """
    ...
    def extract_efficiency_metrics(self, final_metrics: Dict) -> Dict:
        """Extract efficiency metrics (may be empty for standard DETR)."""
        eff_dict = {}
    ...
    def get_epoch_wise_metrics(self) -> pd.DataFrame:
        """Extract epoch-wise metrics for training curves."""
    ...
    def create_efficiency_sheet(self, final_metrics: Dict) -> pd.DataFrame:
        """Create efficiency metrics sheet."""
        eff_metrics = self.extract_efficiency_metrics(final_metrics)
        loss_metrics = self.extract_loss_metrics(final_metrics)
        
        data = []
        for k, v in eff_metrics.items():
            data.append({'Metric': k, 'Value': v})
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Experiment name (e.g., "Pretrained-2", "from_scratch")'
    )
    parser.add_argument(
        '--report_path',
        type=str,
    ...
    try:
        generator = ReportGenerator(args.output_dir, args.experiment_name)
