...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # Guard: If no queries or no targets, return empty indices immediately
        if num_queries == 0 or len(tgt_ids) == 0:
        ...
        # Explicitly use target count to avoid ambiguous -1 with 0 elements
        num_targets = tgt_bbox.shape[0]
        C = C.view(bs, num_queries, num_targets).cpu()
...
class SimpleMinsumMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # Explicitly use target count
        num_targets = tgt_bbox.shape[0]
        C = C.view(bs, num_queries, num_targets)
